...
async function openSurface(page: Page, label: string) {
  ...
  await page
    ...
    .getByRole("button", { name: "TeacherIn", exact: true })
    .click();
  const retainedRoutes: Readonly<Record<string, string>> = {
    "工具连接": "/teacher/ai-agent/tools",
    "定时任务": "/teacher/ai-agent/schedules",
  };
  const retainedRoute = retainedRoutes[label];
  if (retainedRoute) {
    await page.goto(retainedRoute);
  } else {
    await page
      .getByRole("group", { name: "TeacherIn 二级导航" })
...
test("skills market supports search, detail, install and use in task", async ({
  ...
  await expect(
    page.getByRole("button", { name: "移除 Skill 作业错因聚类" }),
  ).toBeVisible();
});
...
test("find, create and direct selection keep Skill use inside the new-task draft", async ({
  ...
  await page.getByRole("button", { name: "移除 Skill 查找技能" }).click();
  await page.getByRole("button", { name: "添加任务材料" }).click();
  await page.getByRole("menuitem", { name: /Skill/ }).click();
  await page.keyboard.press("Escape");
  await expect(page.getByRole("button", { name: "添加任务材料" })).toBeFocused();
  await page.getByRole("button", { name: "添加任务材料" }).click();
  await page.getByRole("menuitem", { name: /Skill/ }).click();
  const picker = page.getByRole("dialog", { name: "选择 Skill" });
  await picker.getByRole("textbox", { name: "搜索 Skill" }).fill("Word");
  await picker.getByRole("button", { name: /Word 文档/ }).click();
  await expect(
    page.getByRole("button", { name: "移除 Skill Word 文档" }),
  ).toBeVisible();
...
test("integrated capability navigation hides retained tools, schedules and settings", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto("/");
  const teacherButton = page.getByRole("button", { name: /老师视角/ });
  if (await teacherButton.count()) await teacherButton.click();
  ...
  for (const hiddenDestination of ["工具连接", "定时任务", "设置"]) {
...
test("capability surfaces remain usable in compact desktop without horizontal overflow", async ({
  ...
  const pickerBox = await page
    .getByRole("dialog", { name: "选择 Skill" })
    .boundingBox();
