...
async function openClassMvpWorkBuddy(page: Page) {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/');
  await page.getByRole('button', { name: /老师视角/ }).click();
...
async function openAllTasks(page: Page) {
  ...
  await page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' }).locator('button[aria-current="page"]').click();
...
test('TeacherIn primary entry only expands the secondary navigation', async ({ page }) => {
  await page.setViewportSize({ width: 1000, height: 768 });
  await page.goto('/');
  await page.getByRole('button', { name: /老师视角/ }).click();

  const primaryNavigation = page.getByRole('navigation', { name: '老师视角主导航' });
  const workBuddyEntry = primaryNavigation.getByRole('button', { name: 'TeacherIn', exact: true });
  await expect(page).toHaveURL(/\/teacher\/home$/);
  await workBuddyEntry.click();
  await expect(page).toHaveURL(/\/teacher\/home$/);
  await expect(workBuddyEntry).toHaveAttribute('aria-expanded', 'true');
  await expect.poll(() => page.locator('aside').first().evaluate((element) => element.getBoundingClientRect().width)).toBe(220);

  const secondaryNavigation = primaryNavigation.getByRole('group', { name: 'TeacherIn 二级导航' });
  const myTasks = secondaryNavigation.getByRole('link', { name: '我的任务', exact: true });
  ...
  await expect(myTasks).toHaveAttribute('aria-current', 'page');
  await page.goto('/teacher/ai-agent');
  await expect(page).toHaveURL(/\/teacher\/ai-agent\/new$/);
});
...
test('teacher enters the collapsible WorkBuddy workspace with renamed capability entries @a11y', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/teacher\/ai-agent\/new$/);
  await expect(workBuddyEntry).toHaveAttribute('aria-expanded', 'true');
  await expect(primaryNavigation.getByRole('link', { name: 'TeacherIn', exact: true })).toHaveCount(0);
  ...
  await expect(page.locator('header[aria-label="TeacherIn 任务导航"]')).toBeVisible();

  const secondaryNavigation = primaryNavigation.getByRole('group', { name: 'TeacherIn 二级导航' });
  expect(await secondaryNavigation.getByRole('link').allTextContents()).toEqual(['我的任务', '技能市场', 'AgentIn', '我的文件']);
  await expect(secondaryNavigation.getByRole('link', { name: '我的任务', exact: true })).toHaveAttribute('aria-current', 'page');
  for (const destination of ['我的任务', 'AgentIn', '技能市场', '我的文件']) {
    await expect(secondaryNavigation.getByRole('link', { name: destination, exact: true })).toBeVisible();
  }
  for (const hiddenDestination of ['工具连接', '定时任务']) {
    await expect(secondaryNavigation.getByRole('link', { name: hiddenDestination, exact: true })).toHaveCount(0);
  }
  ...
  const taskTabs = page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' });
  ...
  await page.goto('/teacher/ai-agent/tools');
  ...
  await expect(page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' })).toHaveCount(0);

  await workBuddyEntry.click();
  await expect(secondaryNavigation).toHaveCount(0);
  await expect(workBuddyEntry).toHaveAttribute('aria-expanded', 'false');
  await workBuddyEntry.click();
  await expect(primaryNavigation.getByRole('group', { name: 'TeacherIn 二级导航' })).toBeVisible();
...
test('ideal new task hides Core Context controls and prepares the demo context automatically', async ({ page }) => {
  await openTeacherWorkBuddy(page);

  await expect(page.getByRole('button', { name: /核心上下文/ })).toHaveCount(0);
  await expect(page.getByRole('complementary', { name: '核心上下文' })).toBeHidden();
...
test('published capability destinations use the standard page topbar instead of task tabs', async ({ page }) => {
  await openTeacherWorkBuddy(page);
  const secondaryNavigation = page.getByRole('group', { name: 'TeacherIn 二级导航' });
  const capabilityStage = page.locator('#main-content').locator('..');

  for (const destination of ['技能市场', '我的文件']) {
  ...
  for (const retainedDestination of [
    { path: '/teacher/ai-agent/tools', title: '工具连接' },
    { path: '/teacher/ai-agent/schedules', title: '定时任务' },
  ]) {
    await page.goto(retainedDestination.path);
    await expect(capabilityStage.locator(':scope > header').getByRole('heading', { level: 1, name: retainedDestination.title })).toBeVisible();
    await expect(page.locator('header[aria-label="TeacherIn 任务导航"]')).toHaveCount(0);
  }

  await secondaryNavigation.getByRole('link', { name: '我的任务', exact: true }).click();
  await expect(page.locator('header[aria-label="TeacherIn 任务导航"]')).toBeVisible();
  await expect(capabilityStage.locator(':scope > header')).toHaveCount(0);
});

test('integrated TeacherIn keeps the retained settings surface unpublished', async ({ page }) => {
  await openTeacherWorkBuddy(page);
  const secondaryNavigation = page.getByRole('group', { name: 'TeacherIn 二级导航' });
  await expect(secondaryNavigation.getByRole('link', { name: '设置', exact: true })).toHaveCount(0);
...
test('student navigation does not expose teacher WorkBuddy', async ({ page }) => {
  ...
  await expect(primaryNavigation.getByRole('button', { name: 'TeacherIn', exact: true })).toHaveCount(0);
  await expect(page.getByRole('group', { name: 'TeacherIn 二级导航' })).toHaveCount(0);
});
...
test('task tabs expose distinct hover and persistent active highlights', async ({ page }) => {
  await openTeacherWorkBuddy(page);
  const taskBar = page.locator('header[aria-label="TeacherIn 任务导航"]');
  const taskTabs = page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' });
...
test('new task entry follows the open tabs and keeps its add icon in every state', async ({ page }) => {
  ...
  const [tabBox, entryBox] = await Promise.all([
  ...
  await expect(page.getByRole('button', { name: '展开核心上下文' })).toHaveCount(0);
