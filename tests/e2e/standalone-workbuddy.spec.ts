...
test('legacy standalone URL redirects to the branded TeacherIn URL', async ({ page }) => {
...
test('standalone personal files expose only local product actions', async ({ page }) => {
  ...
  await expect(page.locator('a[href^="/teacher"]')).toHaveCount(0);
  await expect(page.getByText(/组织云盘|教研组/)).toHaveCount(0);
  ...
  await expect(page.getByRole('button', { name: /创建草稿到 TeacherIn|前往 TeacherIn|发布到 TeacherIn/ })).toHaveCount(0);
...
test('signed-out teachers cannot enter standalone app routes', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: '欢迎回到 TeacherIn' })).toBeVisible();
});
