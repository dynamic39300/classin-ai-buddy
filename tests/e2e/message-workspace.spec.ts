...
async function expectMessageShellLocked(
  ...
  if (before[3].scrollHeight > before[3].clientHeight) {
    await expect.poll(() => timeline.evaluate((element) => element.scrollTop))
      .toBeGreaterThan(before[3].scrollTop);
  }
  ...
  if (before[3].scrollHeight > before[3].clientHeight) expect(after[2]).toBeGreaterThan(before[3].scrollTop);
...
test('single-class immersive chats keep their composer fixed for both roles', async ({ page }) => {
  ...
  for (const entry of [
    ...
    await expect(page.getByLabel('TeacherIn 私密协作窗口')).toHaveCount(entry.hasWorkBuddy ? 1 : 0);
  }
});
...
test('teacher always sees conversation management in class and direct chats', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'TeacherIn', exact: true })).toHaveCount(0);
  const sidecar = page.getByLabel('TeacherIn 私密协作窗口');
  await expect(sidecar).toBeVisible();
  await expect(sidecar).toHaveAttribute('data-dismissible', 'false');
  await expect(sidecar.getByRole('button', { name: '关闭 TeacherIn' })).toHaveCount(0);
...
test('message workspace enter and exit transitions stay perceptible without becoming sluggish', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'TeacherIn 退出引导' })).toHaveCount(0);
  ...
  await expect(page.getByRole('region', { name: 'TeacherIn 退出引导' })).toBeVisible();

  await page.getByRole('button', { name: '关闭退出引导' }).click();
  await page.getByRole('button', { name: 'TeacherIn', exact: true }).click();
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'entering');
  await page.clock.runFor(340);
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'immersive');
});
...
test('teacher enters and exits the immersive message workspace without losing context @a11y', async ({ page }) => {
  ...
  await sidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' }).fill('这是一条尚未发送的 TeacherIn 补充要求');
  ...
  const standardWorkBuddyTrigger = page.getByRole('button', { name: 'TeacherIn', exact: true });
  ...
  const exitGuidance = page.getByRole('region', { name: 'TeacherIn 退出引导' });
  ...
  await exitGuidance.getByRole('button', { name: '重新打开 TeacherIn' }).click();
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'immersive');
  const restoredSidecar = page.getByLabel('TeacherIn 私密协作窗口');
  await expect(restoredSidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' })).toHaveValue('这是一条尚未发送的 TeacherIn 补充要求');
  await expect(page.getByRole('button', { name: 'TeacherIn', exact: true })).toHaveCount(0);
  ...
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'standard');
  await expect(composer).toHaveValue('这是一条尚未发送的消息草稿');
  await expect(page.getByLabel('TeacherIn 私密协作窗口')).toHaveCount(0);
  await expect(page.getByText('已退出沉浸模式', { exact: true })).toBeVisible();
  await page.getByRole('region', { name: 'TeacherIn 退出引导' }).getByRole('button', { name: '关闭退出引导' }).click();
  await expect(page.getByRole('region', { name: 'TeacherIn 退出引导' })).toHaveCount(0);
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'standard');
...
test('WorkBuddy exit guidance stays long enough and pauses while the teacher engages it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '退出沉浸模式' }).click();
  await page.clock.runFor(340);
  const guidance = page.getByRole('region', { name: 'TeacherIn 退出引导' });
  ...
  await guidance.getByRole('button', { name: '重新打开 TeacherIn' }).focus();
...
test('teacher can stop showing the WorkBuddy exit guidance until a full page reload', async ({ page }) => {
  ...
  const accessibility = await new AxeBuilder({ page })
    .include('[aria-label="TeacherIn 退出引导"]')
    .analyze();
  expect(accessibility.violations.filter(({ impact }) => impact === 'serious' || impact === 'critical')).toEqual([]);
  ...
  await guidance.getByRole('button', { name: '重新打开 TeacherIn' }).click();
  const shell = page.locator('[data-shell-mode="linear-workbench"]');
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'immersive');
  await page.getByRole('button', { name: '退出沉浸模式' }).click();
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'standard');
...
test('exit guidance never overlaps a still-mounted WorkBuddy sidecar', async ({ page }) => {
  ...
  const shell = page.locator('[data-shell-mode="linear-workbench"]');
  await expect(shell).toHaveAttribute('data-message-shell-mode', 'immersive');
  await expect(page.getByLabel('TeacherIn 私密协作窗口')).toBeVisible();
  await page.evaluate(() => {
    ...
    const captureConflict = () => {
      ...
      const sidecarVisible = document.querySelector('[aria-label="TeacherIn 私密协作窗口"]') !== null;
...
test('teacher reviews a private WorkBuddy reminder and sends one grouped message @a11y', async ({ page }) => {
  ...
  const workBuddyComposer = sidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' });
  ...
  await expect(sent).not.toContainText('TeacherIn');
...
test('expanded WorkBuddy editor keeps the sidecar shell anchored while its content scrolls', async ({ page }) => {
  ...
  await expect(sidecar.getByText('TeacherIn', { exact: true })).toBeVisible();
  await expect(sidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' })).toBeVisible();
});
...
test('teacher turns the weekly teaching plan into a second simulated class notice', async ({ page }) => {
  ...
  await expect(sidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' })).toHaveValue(/你帮我看看本周的教学计划/);
  await sidecar.getByRole('button', { name: '生成消息草稿' }).click();
...
test('WorkBuddy composer grows with the task and scrolls after its visible height limit', async ({ page }) => {
  ...
  await page.getByRole('link', { name: /消息/ }).click();
  const sidecar = page.getByLabel('TeacherIn 私密协作窗口');
  const composer = sidecar.getByRole('textbox', { name: '向 TeacherIn 输入要求' });
...
test('public chat and private WorkBuddy share one aligned composer contract', async ({ page }) => {
  ...
  const privateComposer = composers.filter({ has: page.getByRole('textbox', { name: '向 TeacherIn 输入要求' }) });
...
test('teacher resizes the WorkBuddy auxiliary workspace with an accessible separator', async ({ page }) => {
  ...
  const separator = page.getByRole('separator', { name: '调整 TeacherIn 宽度' });
...
test('compact immersive messaging keeps WorkBuddy as an overlay without a splitter', async ({ page }) => {
  ...
  await expect(sidecar.getByRole('button', { name: '关闭 TeacherIn' })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'TeacherIn' })).toHaveCount(0);
  await expect(page.getByRole('separator', { name: '调整 TeacherIn 宽度' })).not.toBeVisible();
...
test('teacher approves a guided explanation, opens it from IM and finds the same artifact in My Files @a11y', async ({ page }) => {
  ...
  await primaryNavigation.getByRole('group', { name: 'TeacherIn 二级导航' }).getByRole('link', { name: '我的文件', exact: true }).click();
...
test('teacher and student use isolated direct threads with the same class Agent @a11y', async ({ page }) => {
  ...
  for (const { perspective, ownThread, hiddenThread } of cases) {
    ...
    await expect(conversation.getByRole('button', { name: 'TeacherIn', exact: true })).toHaveCount(0);
    const composer = conversation.getByRole('textbox', { name: '输入消息' });
    await composer.fill('第 5 题的方向怎么判断？');
    await conversation.getByRole('button', { name: '发送', exact: true }).click();
