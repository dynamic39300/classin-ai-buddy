...
describe('WorkBuddy quiz activity integration', () => {
  ...
  it('projects the independent task, paper review, activity parameters and draft approval', async () => {
    ...
    await user.click(primaryNavigation.getByRole('button', { name: 'TeacherIn' }));
    await user.click(within(primaryNavigation.getByRole('group', { name: 'TeacherIn 二级导航' })).getByRole('link', { name: '我的任务' }));
    await user.click(screen.getByRole('button', { name: 'Skill 生成测验' }));
    expect(screen.getByRole('button', { name: '创建任务' })).toBeEnabled();
    await user.click(screen.getByRole('button', { name: '创建任务' }));

    const composer = screen.getByRole('textbox', { name: '向 TeacherIn 补充要求' });
    ...
    expect(screen.getByText('TeacherIn 已收到')).toBeVisible();
