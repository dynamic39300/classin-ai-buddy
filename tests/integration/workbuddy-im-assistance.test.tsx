...
describe('WorkBuddy IM assistance', () => {
  it('generates, approves, distributes and opens a format-neutral guided explanation', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'TeacherIn' }));
    const sidecar = screen.getByLabelText('TeacherIn 私密协作窗口');
  ...
  it('keeps WorkBuddy private to teachers in class chat', () => {
    ...
    expect(screen.getByRole('button', { name: 'TeacherIn' })).toBeInTheDocument();

    unmount();
    renderWorkspace('student-family');
    expect(screen.queryByRole('button', { name: 'TeacherIn' })).not.toBeInTheDocument();
    expect(screen.queryByLabelText('TeacherIn 私密协作窗口')).not.toBeInTheDocument();
  });
  ...
  it('keeps WorkBuddy persistent across teacher immersive class and direct chats', async () => {
    ...
    const sidecar = await screen.findByLabelText('TeacherIn 私密协作窗口');
    expect(sidecar).toHaveAttribute('data-dismissible', 'false');
    expect(within(sidecar).queryByRole('button', { name: '关闭 TeacherIn' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'TeacherIn' })).not.toBeInTheDocument();
  ...
  it('closes WorkBuddy after exiting immersive even when it was already open on entry', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'TeacherIn' }));
    expect(screen.getByLabelText('TeacherIn 私密协作窗口')).toHaveAttribute('data-dismissible', 'true');

    view.rerender(createWorkspaceTree('teacher', undefined, true));
    expect(screen.getByLabelText('TeacherIn 私密协作窗口')).toHaveAttribute('data-dismissible', 'false');

    view.rerender(createWorkspaceTree('teacher'));
    await waitFor(() => expect(screen.queryByLabelText('TeacherIn 私密协作窗口')).not.toBeInTheDocument());
  });
  ...
  it('uses the current teacher direct chat to prepare a reply without sending it', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'TeacherIn' });
  ...
  it('closes the private sidecar with Escape and restores the WorkBuddy trigger', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'TeacherIn' });
    await user.click(trigger);
    expect(screen.getByLabelText('TeacherIn 私密协作窗口')).toBeInTheDocument();
    await user.keyboard('{Escape}');
    await waitFor(() => expect(screen.queryByLabelText('TeacherIn 私密协作窗口')).not.toBeInTheDocument());
  ...
  it('generates, revises, approves and sends one grouped reminder as the teacher', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'TeacherIn' }));
    const sidecar = screen.getByLabelText('TeacherIn 私密协作窗口');
    expect(within(sidecar).getByText('TeacherIn')).toBeInTheDocument();
    ...
    expect(within(sidecar).getByText(/\[模拟\] TeacherIn 评价事件.*尚不代表教学效果/)).toBeInTheDocument();
    ...
    expect(sentMessage).not.toHaveTextContent('TeacherIn');
  ...
  it('creates a second simulated task from the weekly teaching plan and sends one teacher notice', async () => {
    ...
    const composer = within(sidecar).getByRole('textbox', { name: '向 TeacherIn 输入要求' });
  ...
  it('retains both failed and successful execution evidence when a recoverable send succeeds on retry', async () => {
    ...
    expect(within(sidecar).getAllByText(/\[模拟\] TeacherIn 评价事件/)).toHaveLength(2);
  });
});
