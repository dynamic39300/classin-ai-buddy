...
async function expectWorkbenchGeometry(page: Page) {
  const geometry = await page.evaluate(() => {
    ...
    const secondaryNavigation = document.querySelector<HTMLElement>('[role="group"][aria-label="TeacherIn 二级导航"]');
    const workspace = document.querySelector<HTMLElement>('#main-content');
    const workSurface = document.querySelector<HTMLElement>('[aria-label="TeacherIn 工作区"]');
    const stage = document.querySelector<HTMLElement>('[data-workbuddy-stage="true"]');
    const taskBar = document.querySelector<HTMLElement>('header[aria-label="TeacherIn 任务导航"]');
...
test('WorkBuddy new task at 1440x900', async ({ page }) => {
  ...
  for (const title of ['我的任务', 'AgentIn', '技能市场', '我的文件']) {
    await expect(secondaryNavigation.getByRole('link', { name: title, exact: true })).toBeVisible();
  }
  for (const title of ['工具连接', '定时任务', '设置']) {
    await expect(secondaryNavigation.getByRole('link', { name: title, exact: true })).toHaveCount(0);
  }

  await expect(page).toHaveScreenshot('workbuddy-new-task-1440x900.png', { fullPage: true, maxDiffPixels: 300 });
});

test('ClassIn MVP WorkBuddy keeps its full-screen shell with homepage navigation contents at 1440x900', async ({ page }) => {
  ...
  const secondaryNavigation = page.getByRole('navigation', { name: 'TeacherIn 导航' });
  ...
  await expect(page.getByRole('heading', { level: 1, name: '老师好，有什么能帮您的？' })).toBeVisible();
  await expect(secondaryNavigation.getByRole('link', { name: '我的任务', exact: true })).toHaveAttribute('aria-current', 'page');
  for (const title of ['我的任务', '技能市场', 'AgentIn', '我的文件']) {
    await expect(secondaryNavigation.getByRole('link', { name: title, exact: true })).toBeVisible();
  }
  for (const title of ['工具连接', '定时任务', '设置']) {
    await expect(secondaryNavigation.getByRole('link', { name: title, exact: true })).toHaveCount(0);
  }
  await expect(page.getByRole('region', { name: '常用 Agent 和 Skill' })).toBeVisible();
  await expect(page.getByRole('button', { name: '展开核心上下文' })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Skill 生成单个课件' })).toBeVisible();
  await expect(page.getByRole('region', { name: '入口上下文' })).toBeVisible();
...
test('ClassIn MVP WorkBuddy remains reachable at 1024x640', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'TeacherIn 导航' }).getByRole('link', { name: '我的任务', exact: true })).toHaveAttribute('aria-current', 'page');
  await expect(page.getByRole('link', { name: '返回高二物理 3 班' })).toBeVisible();
  await expect(page.getByRole('region', { name: '常用 Agent 和 Skill' })).toBeVisible();
...
test('WorkBuddy new task entry hover at 1440x900', async ({ page }) => {
  await openTeacherAgent(page);
  await expectTypewriterComplete(page);
  const newTaskEntry = page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' }).getByRole('button', { name: '添加新任务' });
...
test('Class-scoped TeacherIn keeps Core Context hidden on the shared task homepage at 1440x900', async ({ page }) => {
  await openClassMvpWorkBuddy(page);
  await expect(page.getByRole('button', { name: '展开核心上下文' })).toHaveCount(0);
  await expect(page.getByRole('complementary', { name: '核心上下文' })).toHaveCount(0);

  await expect(page).toHaveScreenshot('workbuddy-new-task-context-open-1440x900.png', { fullPage: true });
});

test('Class-scoped TeacherIn uses the shared layered task-material picker at 1440x900', async ({ page }) => {
  await openClassMvpWorkBuddy(page);
...
test('WorkBuddy capability page uses the standard title topbar at 1440x900', async ({ page }) => {
  await openTeacherAgent(page);
  await page.getByRole('group', { name: 'TeacherIn 二级导航' }).getByRole('link', { name: '技能市场', exact: true }).click();
...
test('WorkBuddy current Session uses an inline rename field at 1440x900', async ({ page }) => {
  await openTeacherAgent(page);
  await switchTask(page, '生成函数单调性课件');
  const taskTab = page.getByRole('navigation', { name: '已打开的 TeacherIn 任务' }).getByRole('button', { name: '生成函数单调性课件', exact: true });
  await taskTab.hover();
...
test('WorkBuddy all-task selector anchors to the current task tab at 1440x900', async ({ page }) => {
  ...
  const currentTab = page
    .getByRole('navigation', { name: '已打开的 TeacherIn 任务' })
...
test('WorkBuddy M4 course package partial result at 1440x900', async ({ page }) => {
  ...
  await page.clock.pauseAt(new Date(now.getTime() + 1_000));
  await openTeacherAgent(page);
  await page.getByRole('button', { name: 'Skill 生成课程方案包' }).click();
  await expect(page.getByRole('button', { name: '创建任务' })).toBeEnabled();
  await page.getByRole('button', { name: '创建任务' }).click();
  await page.clock.runFor(2_000);
  const timeline = page.getByRole('feed', { name: 'Agent 任务时间线' });
...
test('WorkBuddy keeps embedded navigation reachable at compact desktop width', async ({ page }) => {
  ...
  for (const { name, exact } of [
    { name: '我的任务', exact: true },
    { name: 'AgentIn', exact: true },
  ...
  const geometry = await page.evaluate(() => {
    ...
    const workspace = document.querySelector<HTMLElement>('[aria-label="TeacherIn 工作区"]');
  ...
  await expect(page).toHaveScreenshot('workbuddy-embedded-navigation-1000x768.png', { fullPage: true, maxDiffPixels: 300 });
