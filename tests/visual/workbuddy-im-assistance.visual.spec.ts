...
async function openReminderDraft(page: Page, viewport: { width: number; height: number }) {
  ...
  await page.getByLabel('TeacherIn 私密协作窗口').getByRole('button', { name: '生成消息草稿' }).click();
  await expect(page.getByLabel('TeacherIn 私密协作窗口').getByText('2 项作业', { exact: true })).toBeVisible({ timeout: 12_000 });
}
...
test('WorkBuddy IM reminder draft at 1440x900', async ({ page }) => {
  ...
  await expect(page.getByRole('separator', { name: '调整 TeacherIn 宽度' })).toBeVisible();
...
test('WorkBuddy guided explanation review remains reachable at 1024x640', async ({ page }) => {
  ...
  const sidecarBody = page.getByLabel('TeacherIn 私密协作窗口').locator('[data-scrolled]');
