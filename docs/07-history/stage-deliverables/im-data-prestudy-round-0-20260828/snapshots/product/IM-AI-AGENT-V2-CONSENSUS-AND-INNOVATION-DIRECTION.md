...
status: ALIGNMENT_BASELINE_WITH_INDUSTRY_RESEARCH
version: v0.3
date: 2026-08-28
...
2. 当前项目目录中可运行的 IM + TeachBuddy + `@Agent` 候选 Demo；
...
| D1 | [项目简报](../00-project/PROJECT-BRIEF.md)与[决策台账](../00-project/DECISION-LEDGER.md) | TeachBuddy、教师主 Agent、事实所有权和审批等锁定边界 |
...
- 老师专属的 TeachBuddy/WorkBuddy 辅助区；
...
- 增加公开群内 `@Agent`、隔离 Agent 私聊和教师私密 TeachBuddy；
...
- TeachBuddy 是教师统一主 Agent/AI 教学搭档，不要求教师理解内部 Agent、Skill 或模型；
- TeachBuddy 的私密草稿、Run 和工具过程不能泄漏给学生；
- ClassIn 继续拥有班级、课程、作业、消息和正式业务结果；
- AI 业务副作用必须经过 ProposedAction、Approval 和 ExecutionReceipt；
- 公开班级 Agent 与教师私密 TeachBuddy 是不同产品身份，不能因代码复用而混同；
...
> **ClassIn 班级学习协作场**：以班级和师生关系为上下文，以沟通为连接基础，以问题、讨论、活动、反馈和成果为核心对象，由 TeachBuddy 与班级 Agent 在清晰权限下帮助老师组织学习、促进参与并形成可见结果。
...
|---|---|---|---|---|
| 方案一：AI Enhanced IM | 以 1.0 IM 为基础，增强消息基建、`@Agent`、Agent 私聊和教师 TeachBuddy 辅助 | 在现有 Demo 能力上继续深化，可逐步演进 | 已有可运行 Demo，可继续优化 | AI 如何更自然融入单聊/群聊；哪些基础 IM 能力真正值得补齐 |
...
### 6.4 TeachBuddy 教师私密层

TeachBuddy 不只是聊天侧边栏，而是老师管理学习现场的私密控制层：
...
它不能读取教师私密 TeachBuddy 内容，也不能以教师身份发言。
...
| Micro Activity | 投票、快问快答、练习、反思等轻活动 | 老师/TeachBuddy 创建 | 待发布/进行中/已结束 |
| Intervention Proposal | AI 给老师的干预建议 | TeachBuddy | 待审/已修改/拒绝/批准 |
...
|---|---|---|---|---|
| TeachBuddy | 老师 | 教师私密 | 理解班级、提出建议、生成 Artifact、准备行动、复盘 | 未经确认公开发言或暴露学生推断 |
...
| L1 静默理解 | 后台聚类问题、识别未回应和参与变化 | 仅进入 TeachBuddy 私密层 | 班级级开关 + 最小数据范围 |
...
多名学生在群里用不同表达询问相似问题。TeachBuddy 在后台聚类，并私下告诉老师：
...
- 可与当前 Message、TeachBuddy Run、Approval、Delivery 和 Receipt 能力衔接。

### 10.3 与当前 V2-IM-01 的关系

当前蓝图推荐的“学生私聊问题 → 教师 TeachBuddy 讲题 → 审批发送”仍然有价值，它是安全、清晰的基础能力纵向切片。
...
5. **身份与可见性始终明确**：老师、TeachBuddy、班级 Agent 和系统内容不可混淆。
...
1. **增强型聊天**：验证 `@Agent + TeachBuddy` 的最低迁移成本；
...
- 入口与退出；
- Message、Learning Moment、Agent Turn 和 TeachBuddy Run 的关系；
- 权限、主动性和审批；
- 状态、失败和恢复；
- 所需真实 ClassIn 数据/API；
- 老师与学生的验收指标；
