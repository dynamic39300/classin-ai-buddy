---
title: M4.4 独立教师 ClassIn TeacherIn 实现追踪
status: COMPLETE
version: v1.2
date: 2026-08-25
fixed_point: 6508529
...
公开 URL 由 `STANDALONE_TEACHERIN_ROUTES` 统一拥有；`App.ProductComposition` 同时识别主路径与旧 `/workbuddy/*`，后者只进入 `LegacyStandalonePathRedirect`，按原子路径、查询参数和锚点迁移到 `/teachbuddy/*`。E2E 锁定旧官网锚点兼容跳转，所有正常旅程只使用品牌化主路径。
