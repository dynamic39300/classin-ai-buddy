---
title: M4.4 独立教师 ClassIn TeacherIn Web 产品规格包
...
# M4.4 独立教师 ClassIn TeacherIn Web 产品

> 当前正式产品名按 D-132 统一为 **ClassIn TeacherIn**，工作台简称 **TeacherIn**，中文描述为 **AI 教学搭档**。本目录和既有类型中的 `workbuddy` 保留为内部工程兼容标识。

本目录按 `Research → PRD → Feature Spec → Tickets → Implementation → Review → Acceptance` 管理 M4.4。产品面向个人教师和未合作机构中的教师，提供独立官网、个人账号、完整 TeacherIn、AI 点数、模拟会员订单以及连接 ClassIn 的价值引导。
