...
- Skills：广场、我的 Skill、详情与安装/启停/去使用；
- Tools：连接广场、连接状态、测试连接、权限摘要与治理；
- Content：内容广场、筛选、详情、收藏与改编到新任务；
- Files：复用 Space 语义的 WorkBuddy 文件视图、预览和加入任务；
...
8. As a teacher, I want to view WorkBuddy files by source, owner, task and version, so that files do not become a second ownership system.
...
- Integration tests render each surface through the public provider and verify navigation actions, dialogs, empty/error states and accessible names.
...
- Real Skill runtime, MCP process, model provider, content marketplace API, file upload/download backend or durable scheduler.
