---
title: M4.3 ClassIn 站内 TeacherIn MVP 入口 PRD
status: IMPLEMENTED_READY_FOR_REVIEW
version: v1.3
date: 2026-09-04
decision: D-098, D-130, D-131
---

# M4.3 ClassIn 站内 TeacherIn MVP 入口 PRD

## 0. 2026-09-04 当前替代要求

D-130 与 D-131 共同定义当前班级详情卡片入口：

1. 保留班级作用域 URL、受校验的 Launch Context 和 `classin-mvp` 数据 Namespace；
2. 保留原独立全屏 `ClassMvpWorkBuddyShell`、旧导航版式、品牌区、入口班级信息和返回班级命令，不挂载 ClassIn 教师 `AppShell`；
3. 只把导航菜单内容更新为“我的任务、技能市场、AgentIn、我的文件”；
4. “我的任务”正文与 `/teacher/ai-agent/new` 使用同一个 Task Assembly 首页及完整交互，不复制页面实现；
5. 工具连接、定时任务和设置不显示；班级作用域内部链接不跳回 `ideal-full`；
6. 独立 Shell 继续作为卡片入口后的可见页面，班级 Route、Launch Context 和 Session Namespace 继续独立。

下文保留为 v1.2 历史基线；与本节冲突时以本节为准。

## 1. 背景与目标

M4.2 已完成终局 TeacherIn 的一级导航体验与代表性任务闭环。M4.3 在班级课程详情增加首发 MVP 的真实入口：老师从具体教学现场打开 TeacherIn，进入独立的 TeacherIn MVP。初始工程先完成终局方案搬迁，再根据本轮验收形成第一版 MVP 导航裁剪。

本阶段同时保留 Demo 中的终局入口和 MVP 入口，用于对照验证；真实 MVP 发布配置可隐藏终局一级入口。两个入口都代表“老师自己的 TeacherIn”，但属于两个独立产品 Experience，不共享任务数据，避免未来 MVP 删除能力后仍暴露终局历史。
...
| TeacherIn | 当前老师；仅老师可见 | 打开 `classin-mvp` TeacherIn；不属于班级共享 Agent |

TeacherIn 必须是独立 Section，显示 `TeacherIn`、`AI 教学搭档 · 仅你可见` 和“打开 TeacherIn”。它不能混入“AI 应用”列表，也不能使用“授权给班级”或“班级成员可用”的文案。

## 5. 核心旅程

```text
老师进入 A 班课程详情
  → 区分“AI 应用”与“TeacherIn”
  → 打开独立、全屏的 MVP TeacherIn 页面
  → 使用页面自己的 TeacherIn 左侧导航、任务工作台和能力页
...
- `classin-mvp` 使用独立全屏页面，不挂载 ClassIn 原主导航，也不向原主导航新增 TeacherIn 一级入口；页面可见名称统一为 `TeacherIn`。
...
- 学生页面、学生导航和班级共享 Agent 列表不得出现教师 TeacherIn 入口。
...
- `M43-PRD-001`：班级详情保留“AI 应用”并新增独立 TeacherIn。
- `M43-PRD-002`：AI 应用继续表达班级授权、成员可用 Agent，不改变既有行为。
- `M43-PRD-003`：MVP TeacherIn 入口仅教师可见，明确为 AI 教学搭档。
- `M43-PRD-004`：点击入口打开独立全屏 TeacherIn，任务闭环沿用已搬迁的终局实现。
...
- `M43-PRD-016`：MVP 使用独立全屏 TeacherIn Shell；原 ClassIn 主导航不挂载、不新增 MVP 入口，返回命令稳定回到来源班级。
...
- 班级详情能清楚区分共享 AI 应用与私密 TeacherIn，并打开完整 MVP 工作区。
- Demo 中终局与 MVP 入口都可使用，视觉与操作规范一致。
- MVP 页面只显示其自身 TeacherIn 导航，不显示 ClassIn“老师视角主导航”或新增的 TeacherIn 一级菜单。
