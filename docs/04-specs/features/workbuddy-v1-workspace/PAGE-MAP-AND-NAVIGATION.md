...
| WB-N-P01 | TeacherIn 二级导航目录 | ClassIn 左侧栏内嵌 Surface | 仅负责展开/收起的 TeacherIn 一级入口 | 我的任务、技能市场、AgentIn、我的文件及其他按 Profile 发布的能力 | G-P01 + ClassIn 适配 |
...
```mermaid
flowchart TD
  SH["WB-G-P01 ClassIn Shell"] --> NAV["WB-N-P01 TeacherIn 二级导航目录"]
...
教师端 `ideal-full` 左侧二级目录当前依次为：我的任务、技能市场、AgentIn、我的文件；工具连接与定时任务只隐藏发布入口，设置继续不发布，旧“内容资源”保留为 Dormant Module。一级 TeacherIn 入口采用与“班课管理”一致的整行展开式交互，只展开或收起二级导航，不再跳转页面，也不产生第三级菜单。“我的任务”继续指向既有 `/teacher/ai-agent/new` 首页；新任务和 Run 均保持该入口选中。
