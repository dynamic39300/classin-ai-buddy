...
这是嵌入 ClassIn 左侧栏 TeacherIn 展开式一级入口下方的二级导航扩展。`ideal-full` 首项为“我的任务”，指向既有新任务工作台；其后按 Experience Profile 投影 Skills、AgentIn、Tools、内容、定时任务、文件与设置等已发布 Capability。TeacherIn 一级入口只展开或收起本导航，不直接跳转页面；“我的任务”在新任务与 Run 路由保持选中。所有目的地保持同一导航层级，不渲染第三级菜单；右侧 Work Surface 不为导航预留宽度。
