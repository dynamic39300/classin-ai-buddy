---
title: ClassIn TeacherIn 当前状态与下一阶段计划
status: TEACHERIN_GLOBAL_DISPLAY_NAME_MIGRATED
version: v0.26
date: 2026-09-04
---
...
> D-132 已锁定当前展示品牌：正式名称为 **ClassIn TeacherIn**，界面简称 **TeacherIn**，中文描述为 **AI 教学搭档**。下文 `WorkBuddyRun` 等 PascalCase 名称及 `workbuddy` 路径仍表示内部工程兼容标识；阶段验收原始名称仅用于历史追踪。

## 0. 当前进行项（2026-09-04）

全局产品字面名称迁移已按 D-132 实施：页面入口、工作台、班级 Banner、IM 私密协作、独立产品、浏览器元数据、ARIA、Mock 展示文案与自动化断言统一使用 **TeacherIn**；现行项目简报、术语、研究、产品方案、PRD、Feature Spec、Ticket、工程说明和阶段报告同步更新。品牌契约改为 `TEACHERIN_BRAND`，头像组件改为 `TeacherInAvatar`。`WorkBuddy` / `workbuddy`、已发布 `/teachbuddy/*`、存储键、数据属性和历史证据继续作为兼容标识保留。完整阶段记录见 [2026-09-04 TeacherIn 全局命名迁移里程碑](./MILESTONE-2026-09-04-TEACHERIN-GLOBAL-NAMING.md)。

本轮验证已完成：现行运行时代码旧展示名扫描为 0；TypeScript、99 个 Vitest 文件 / 627 项测试、production build、4 条关键 Chromium E2E 与 6 组关键视觉检查通过。全仓 Lint 继续受研究工具目录 43 个既有规则错误阻断，应用与测试范围 ESLint 为 0 error；该既有债务未纳入品牌迁移。

### 上一阶段（2026-09-03）

M4.5 已完成用户验收。`ideal-full` TeacherIn 新任务首页任务装配升级已完成 NTA-01～NTA-13 实施与范围验证。本轮交付 5 个常用 Skill、4 个常用 Agent、三类文件来源、独立文件选择弹窗、统一任务材料区、上方逐级添加选择器、最新自动 Prompt 替换、WorkBuddy 式 56rem 居中内容轴与完整标签停靠，以及与三类 Run 关联的稳定 TaskAssemblySnapshot；原四个任务快捷按钮迁入常用 Skill。唯一入口见 [新任务首页任务装配升级](../04-specs/features/workbuddy-new-task-assembly/README.md)。

用户于 2026-09-03 继续校准教师端信息架构：TeacherIn 一级入口按 D-127 改为与“班课管理”一致的整行展开式导航组，不再直接承载或跳转首页；新增首个二级入口“我的任务”，继续进入既有 `/teacher/ai-agent/new` 首页，其后为“技能市场 → AgentIn → 我的文件”。直接深链、Run、Capability、`classin-mvp`、独立产品和学生端均保持不变。

范围证据：627 项 Vitest、TypeScript、`src/` + `tests/` ESLint、production build 均通过；D-125 定向 Chromium E2E 4 / 4、新任务装配视觉 8 / 8 通过，此前 49 项受影响 E2E / Axe 证据继续保留。全仓 `npm run lint` 仍受并行 IM 数据研究工具目录 39 个既有错误阻断；全仓旧视觉基线存在无关漂移，未批量覆盖。完整证据见 [实现 Review](../04-specs/features/workbuddy-new-task-assembly/IMPLEMENTATION-REVIEW.md)。
...
| TeacherIn IM | 作业催交、课前准备通知、沉浸消息工作台和显式审批发送已实现 | 2026-08-24 用户确认 v0.19 验收完成 |
...
| M4.4 独立教师 ClassIn TeacherIn Web 产品 | 第三套独立 Product Module 已完成：独立官网、教师个人账号、完整任务/能力工作台、AI 点数预占/结算/释放、模拟会员订单、无 ClassIn Context 执行及 ClassIn 价值转化；不挂载 ClassIn 业务 Provider，不共享终局/MVP 私有数据；内容资源统一采用 TeacherIn 兼容格式，为未来受治理接入内部内容生态保留零格式转换路径 | 2026-08-25 用户完成方案与页面 Review，`COMPLETE_USER_ACCEPTED` |
...
| TeacherIn 新任务首页任务装配 | 5 Skill + 4 Agent、三类文件、两个独立 Dialog、统一材料区、Prompt 来源与三类 Run Snapshot 已实现 | 工程 Gate 完成，等待用户页面 Review |
