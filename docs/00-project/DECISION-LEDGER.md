...
| D-024 | LOCKED | AI Agent 二级导航作为上下文扩展嵌入 ClassIn 左侧栏的 `AI Agent` 一级入口下方；右侧 Stage 不再保留独立导航列，完整用于 Run、Artifact、Core Context 与执行详情 | 用户完成 M3 功能框架验收后，根据实机截图明确要求释放右侧页面篇幅，同时保持任务历史与能力入口属于 AI Agent 导航 | 两级 IA、扁平 Section、6 条历史和全部能力入口不变；AppShell 只提供扩展槽，WorkBuddy Feature 继续拥有导航逻辑；Agent 路由外收起扩展，紧凑桌面保持左栏展开以确保可达 |
...
| R-003 | RECOMMENDATION | M4 固定 Mock 纵向闭环已达到 Phase 4 用户验收候选：单课件、课程方案包、Core Context、Artifact、审批写回、Receipt、失败恢复、派生 Run 与 Replanning 均可操作 | 自动化与视觉证据证明已达到可审阅状态，但尚未取得用户的 M4 产品验收确认 | 继续保留 `READY_FOR_USER_REVIEW`，不把 Mock Harness 或原型行为误写成生产能力；用户验收后再决定下一阶段与是否新增 LOCKED 决策 |
