...
WorkBuddy 既不能成为脱离 ClassIn 的独立桌面应用，也不能被压缩成普通聊天抽屉。目标结构是两级导航：ClassIn 一级主导航继续表达教师身份、组织和业务对象；教师从 AI Agent 入口进入后，使用一列对标 NineClaw 的扁平二级导航面板和连续 Agent Work Surface。历史、能力、内容和触发渠道围绕同一 Run 模型组织。
...
一级主导航和 AI Agent 二级导航是两列 Shell Surface。二级面板中的“近期任务”“能力与资源”等文字只作为 Section 标题，不形成第三级菜单；任务条目就是二级面板内可直接打开的导航对象。二级面板内部可滚动，但当前任务、输入草稿和右侧活动区不随导航滚动丢失，也不在 Work Surface 左侧再建立第三根历史栏。
