...
这是 ClassIn PC 教师/学生产品基座与教师 AI 教学搭档 **ClassIn TeacherIn** 的统一设计、研究和实现仓库，也是 TeacherIn 2.0 的设计与开发工作区。产品界面简称 **TeacherIn**；内部工程标识继续使用 `workbuddy`，既有 `WorkBuddyRun`、路由和存储键不随品牌展示名迁移。当前代码保留既有 ClassIn PC Demo 的完整业务背景，并在教师端增加 AI Agent 工作台；首条纵向切片是“课程目标到课程对象”。

## 当前入口

- Agent 规范：[AGENTS.md](./AGENTS.md)
- 2.0 迁移基线：[docs/00-project/V2-ITERATION-BASELINE.md](./docs/00-project/V2-ITERATION-BASELINE.md)
...
- TeacherIn V1 规格（目录保留内部工程标识）：[docs/04-specs/features/workbuddy-v1-workspace/](./docs/04-specs/features/workbuddy-v1-workspace/)
