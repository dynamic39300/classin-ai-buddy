...
export type QuizActivityCreationRun = Readonly<{
  ...
  goal: string;
  contextSnapshotId: string;
  taskAssemblySnapshotId: string | null;
  target: QuizActivityTarget;
...
function create(input: Readonly<{ runId: string; contextSnapshotId: string; taskAssemblySnapshotId?: string | null; goal: string; target: QuizActivityTarget; now: string }>): QuizActivityCreationRun {
  ...
  return freezeRun({
    fixtureVersion: 'workbuddy-quiz-activity-v1', id: input.runId, taskType: 'quiz-activity-creation', goal: input.goal.trim(), contextSnapshotId: input.contextSnapshotId, taskAssemblySnapshotId: input.taskAssemblySnapshotId ?? null,
