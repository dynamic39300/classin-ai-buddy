...
describe('QuizActivityCreationModule', () => {
  it('retains the immutable task-assembly snapshot reference', () => {
    const run = QuizActivityCreationModule.create({
      runId: 'run-snapshot', contextSnapshotId: 'context-1', taskAssemblySnapshotId: 'assembly-1', goal: '生成测验',
      target: { classId: 'class-1', courseId: 'course-1', unitId: 'unit-1', expectedVersion: 'v1', label: '目标' },
      now: '2026-09-03T00:00:00.000Z',
    });
    expect(run.taskAssemblySnapshotId).toBe('assembly-1');
  });
