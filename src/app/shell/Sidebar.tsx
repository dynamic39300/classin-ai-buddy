            ...
            {navigation.filter((node) => node.group === group).map((node) => (
              <Fragment key={node.id}>
                ...
                {navigationExtension?.afterItemId === node.id && navigationExtensionOpen ? (
                  <div className={styles.extensionContent} id={`${node.id}-extension`}>
                    {navigationExtension.content}
                  </div>
                ) : null}
              </Fragment>
            ))}
...
function NavigationNodeView({ node, open, disableCollapse, onNavigate, onToggle, hasExtension, extensionParentBehavior, extensionOpen, onToggleExtension, forceActive }: NavigationNodeViewProps) {
  if (node.kind === 'item') {
      ...
      return (
        <div className={styles.collapsibleNode}>
          ...
          >
            ...
            <ChevronRight
              className={`${styles.chevron} ${extensionOpen ? styles.chevronExpanded : ''}`}
              aria-hidden="true"
              size={16}
            />
          </button>
