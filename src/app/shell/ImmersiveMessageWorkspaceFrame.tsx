...
function hasLocalEscapeSurface(): boolean {
  return Boolean(document.querySelector(
    `dialog[open], [role="dialog"], [role="menu"], [aria-label="${TEACHERIN_BRAND.shortName} 私密协作窗口"][data-dismissible="true"]`,
  ));
}
...
export function ImmersiveMessageWorkspaceFrame({
  ...
  const requestExitWithGuidance = () => {
    resetEscape();
    const workBuddyWasVisible = Boolean(document.querySelector(`[aria-label="${TEACHERIN_BRAND.shortName} 私密协作窗口"]`));
  ...
  return (
    ...
    >
      {shellVisible ? (
        <header className={styles.toolbar} aria-label={`${title}沉浸工作区导航`}>
          <div className={styles.location}>
            <span className={styles.brandMark} aria-hidden="true"><img alt="" src={publicAssetUrl('brand/classin-wing-mark.png')} /></span>
            <h1>{title}</h1>
          </div>
      ...
      {exitGuidanceVisible ? (
        <section
          aria-label={`${TEACHERIN_BRAND.shortName} 退出引导`}
        ...
        >
          ...
          <div className={styles.exitGuidanceContent}>
            ...
            <div className={styles.exitGuidanceActions}>
              <button type="button" onClick={reopenWorkBuddy}><Sparkles aria-hidden="true" size={14} />重新打开 {TEACHERIN_BRAND.shortName}</button>
