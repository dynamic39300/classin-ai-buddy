...
describe('teacher navigation architecture', () => {
  ...
  it('exposes TeacherIn only in the teacher tree and keeps every nested route active', () => {
    expect(getNavigation('teacher').find(({ id }) => id === 'teacher-ai-agent')).toMatchObject({ label: 'TeacherIn' });
