...
import { createDemoClassInSpaceAdapter, createDemoMyFilesAdapter } from '@mocks/adapters/task-material-source';
...
import { isStandaloneTeacherInPath } from '@contracts/workbuddy/product-brand';
import { applicationBasename } from '@shared/public-assets';
...
function ClassInWorkBuddyBridge({ children }: { children: ReactNode }) {
  ...
  const adapters = useMemo(() => {
    ...
    return {
      ...
      taskMaterialCatalogs: {
        myFiles: createDemoMyFilesAdapter(),
        classInSpace: createDemoClassInSpaceAdapter(),
      },
  ...
  return (
    <WorkBuddyWorkspaceProvider
      ...
      taskMaterialCatalogs={adapters.taskMaterialCatalogs}
    >
      {children}
    </WorkBuddyWorkspaceProvider>
  );
...
function ProductComposition() {
  ...
  return isStandaloneTeacherInPath(pathname)
