...
export function StandaloneCreditsPage() {
  ...
  return <section className={styles.commercePage} aria-labelledby="credits-title"><header><div><span className={styles.kicker}>AI 点数账户</span><h1 id="credits-title">每一次生成，都清楚可追溯</h1><p>提交任务前显示固定报价；只有任务成功创建才会结算。体验点数当前不会按月自动重置。</p></div><Link to="/workbuddy/app/membership">补充 AI 点数</Link></header><div className={styles.balancePanel} aria-live="polite"><span>可用点数</span><strong>{creditView?.availableBalance ?? 0}</strong><small>[模拟] AI 点数</small><dl><div><dt>账户余额</dt><dd>{creditView?.balance ?? 0}</dd></div><div><dt>任务预占</dt><dd>{creditView?.reserved ?? 0}</dd></div></dl></div><section className={styles.ledger} aria-labelledby="ledger-title"><h2 id="ledger-title">点数流水</h2>{creditView?.ledger.length ? <ul>{creditView.ledger.map((entry) => <li key={entry.id}><span><strong>{labels[entry.kind]}</strong><small>{entry.truthLabel} · 余额 {entry.balanceAfter}</small></span><em data-positive={entry.amount > 0}>{entry.amount > 0 ? '+' : ''}{entry.amount}</em></li>)}</ul> : <p>暂无点数流水。</p>}</section></section>;
}
...
function MembershipDialog({ plan, onCancel, onConfirm }: Readonly<{ plan: MembershipPlan; onCancel: () => void; onConfirm: (planId: MembershipPlanId) => void }>) {
  ...
  return <dialog className={styles.orderDialog} ref={ref} onCancel={(event) => { event.preventDefault(); onCancel(); }}><form method="dialog"><span className={styles.kicker}>[模拟] 会员订单</span><h2>确认选择{plan.name}</h2><p>本次不会产生真实扣款。确认后将立即模拟到账，用于完整体验任务点数逻辑。</p><dl><div><dt>套餐金额</dt><dd>¥{plan.priceCny} / 月</dd></div><div><dt>到账点数</dt><dd>{plan.credits} AI 点数</dd></div></dl><div><button type="button" onClick={onCancel}>取消</button><button autoFocus type="button" onClick={() => onConfirm(plan.id)}>确认模拟到账</button></div></form></dialog>;
}
