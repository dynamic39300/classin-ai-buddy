  ...
  return (
    <section className={styles.classInPage} aria-labelledby="classin-connection-title">
      <header>
        ...
        <p>当前 {TEACHERIN_BRAND.shortName} 只依据你主动输入的任务描述和上传资料。它不会读取任何班级、学生或作业事实。</p>
      </header>
      ...
      <aside className={styles.classInCta}>
        <div><strong>你不需要先连接，才能体验 {TEACHERIN_BRAND.shortName}</strong><p>先完成一个任务；需要跨班级上下文、教学活动写回或机构协同时，再了解 ClassIn 的完整方案。</p></div>
        <div><Link to={STANDALONE_TEACHERIN_ROUTES.newTask}>先创建一个任务</Link><a href="https://www.classin.com/" target="_blank" rel="noreferrer">了解 ClassIn <ArrowUpRight size={15} /></a></div>
      </aside>
    </section>
  );
}
