    ...
    <div className={styles.marketingPage}>
      ...
      <main>
        ...
        <section className={styles.pointsStory} id="plans" aria-labelledby="points-title">
          <div><span className={styles.kicker}>清晰可控的 AI 点数</span><h2 id="points-title">先看到消耗，再开始任务</h2><p>每类任务都有固定模拟报价。创建成功才结算，未创建会释放预占点数，流水可随时核对。</p><Link to="/workbuddy/register">领取 360 点免费体验</Link></div>
...
function safeNext(value: string | null): string {
  return value?.startsWith('/workbuddy/app/') ? value : '/workbuddy/app/new';
}
