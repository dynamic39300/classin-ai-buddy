...
const PRODUCT_PREVIEW = publicAssetUrl('brand/workbuddy-standalone-product-preview.png');
  ...
  return (
    <div className={styles.marketingPage}>
      <header className={styles.marketingHeader}>
        <Link className={styles.wordmark} to={STANDALONE_TEACHERIN_ROUTES.root}><span><Sparkles size={18} /></span><strong>{TEACHERIN_BRAND.officialName}</strong><small>{TEACHERIN_BRAND.descriptor}</small></Link>
        <nav aria-label="官网导航"><a href="#capabilities">教学能力</a><a href="#plans">AI 点数</a><a href="#classin">连接 ClassIn</a></nav>
        <div className={styles.headerActions}>{identity.status === 'signed_in' ? <Link to={STANDALONE_TEACHERIN_ROUTES.newTask}>进入工作台</Link> : <><Link to={STANDALONE_TEACHERIN_ROUTES.login}>登录</Link><Link data-primary="true" to={STANDALONE_TEACHERIN_ROUTES.register}>免费开始</Link></>}</div>
      </header>
      ...
      <main>
        <section className={styles.hero}>
          <div className={styles.heroCopy}>
            ...
            <p>从一节课、一套课程方案到一份测验，{TEACHERIN_BRAND.shortName} 通过对话理解你的目标，生成可检查、可修改的教学产物。</p>
            <div className={styles.heroActions}><Link data-primary="true" to={identity.status === 'signed_in' ? STANDALONE_TEACHERIN_ROUTES.newTask : STANDALONE_TEACHERIN_ROUTES.register}>免费开始 <span aria-hidden="true">→</span></Link><a href="#classin">了解 ClassIn</a></div>
          ...
          <figure className={styles.productVisual}><img alt={`${TEACHERIN_BRAND.officialName} 教师工作台的新建任务页面`} src={PRODUCT_PREVIEW} /><figcaption><span>产品体验</span><strong>一个入口，承接完整教学任务</strong></figcaption></figure>
        </section>
        ...
        <section className={styles.pointsStory} id="plans" aria-labelledby="points-title">
          <div><span className={styles.kicker}>清晰可控的 AI 点数</span><h2 id="points-title">先看到消耗，再开始任务</h2><p>每类任务都有清晰报价。创建成功才结算，未创建会释放预占点数，流水可随时核对。</p><Link to={STANDALONE_TEACHERIN_ROUTES.register}>领取 360 点免费体验</Link></div>
      ...
      <footer className={styles.marketingFooter}><span>{TEACHERIN_BRAND.officialName}</span><span>面向教师的独立 AI 教学工作台</span></footer>
    </div>
  );
}

function safeNext(value: string | null): string {
  return value?.startsWith(`${STANDALONE_TEACHERIN_ROUTES.app}/`) ? value : STANDALONE_TEACHERIN_ROUTES.newTask;
}
...
export function StandaloneAuthPage({ mode }: Readonly<{ mode: 'login' | 'register' }>) {
  ...
  return (
    <main className={styles.authPage}>
      <Link className={styles.authBrand} to={STANDALONE_TEACHERIN_ROUTES.root}><span><Sparkles size={20} /></span><strong>{TEACHERIN_BRAND.officialName}</strong><small>{TEACHERIN_BRAND.descriptor}</small></Link>
      <section className={styles.authPanel} aria-labelledby="auth-title">
        <span className={styles.kicker}>教师个人账号</span>
        <h1 id="auth-title">{mode === 'register' ? '开始你的 AI 教学工作台' : `欢迎回到 ${TEACHERIN_BRAND.shortName}`}</h1>
        ...
        <p className={styles.authSwitch}>{mode === 'register' ? '已经有账号？' : '还没有账号？'} <Link to={`${mode === 'register' ? STANDALONE_TEACHERIN_ROUTES.login : STANDALONE_TEACHERIN_ROUTES.register}?next=${encodeURIComponent(next)}`}>{mode === 'register' ? '直接登录' : '免费注册'}</Link></p>
        <small>注册即得 360 AI 点，开启你的教学工作台。</small>
      </section>
      <aside className={styles.authAside}><span>教师的一天，少一点重复工作</span><blockquote>“先让我看清楚结果，再决定是否使用。”</blockquote><p>{TEACHERIN_BRAND.shortName} 为每个任务保留清晰的上下文、生成过程、审阅动作和结果证据。</p></aside>
    </main>
  );
}
