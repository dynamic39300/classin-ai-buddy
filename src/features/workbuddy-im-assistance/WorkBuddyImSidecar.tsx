...
import { TeacherInAvatar } from '@design-system/TeacherInAvatar';
...
const TEACHERIN_GREETING = '我是您的教学搭档，有什么要帮忙？';
const GREETING_SEEN_KEY = 'classin:teachbuddy:im-greeting-seen:v1';

function useTeacherInGreeting(): string {
  const [greeting, setGreeting] = useState(() => {
    if (typeof window === 'undefined') return TEACHERIN_GREETING;
  ...
  useEffect(() => {
    if (greeting === TEACHERIN_GREETING || typeof window === 'undefined') return undefined;
    window.sessionStorage.setItem(GREETING_SEEN_KEY, 'true');
    const timer = window.setInterval(() => {
      setGreeting((current) => TEACHERIN_GREETING.slice(0, current.length + 1));
    }, 45);
...
export function WorkBuddyImSidecar({ onLocateMessage, onInsertDirectReply, onClose }: WorkBuddyImSidecarProps) {
  ...
  const greeting = useTeacherInGreeting();
  ...
  const composerActionLabel = run.status === 'ready'
    ...
    : run.status === 'generating' ? '发送补充要求' : `发送给 ${TEACHERIN_BRAND.shortName}`;
    ...
    : run.status === 'sending'
      ...
      : run.status === 'draft-ready'
        ...
        : run.status === 'direct-draft-ready'
          ...
          : directContext
            ? '内容仅用于当前私聊的教师回复辅助'
            : `内容仅进入教师与 ${TEACHERIN_BRAND.shortName} 的私密任务窗口`;
  ...
  return (
    <aside className={styles.sidecar} aria-label={`${TEACHERIN_BRAND.shortName} 私密协作窗口`} data-dismissible={onClose ? 'true' : 'false'} data-surface="floating-assistant" id="workbuddy-im-sidecar">
      <header className={styles.header}>
        <div className={styles.identity}>
          <TeacherInAvatar size="compact" />
          <strong>{TEACHERIN_BRAND.shortName}</strong>
          <span aria-label={TEACHERIN_GREETING} className={styles.greeting}>{greeting}{greeting !== TEACHERIN_GREETING ? <i aria-hidden="true" /> : null}</span>
        </div>
        <div className={styles.headerActions}>
          <span className={styles.context}><small>当前上下文</small><strong title={target.classLabel}>{target.classLabel}</strong></span>
          {onClose ? <button type="button" aria-label={`关闭 ${TEACHERIN_BRAND.shortName}`} onClick={onClose}><X aria-hidden="true" size={17} /></button> : null}
        </div>
      </header>
      ...
      <WorkspaceComposer
        ariaLabel={`向 ${TEACHERIN_BRAND.shortName} 输入要求`}
        ...
        placeholder={run.status === 'generating' ? '补充要求或调整语气…' : directContext ? '例如：帮我拟一条简洁、专业的回复…' : `给 ${TEACHERIN_BRAND.shortName} 安排任务或继续追问…`}
