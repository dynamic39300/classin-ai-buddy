  ...
  return (
    <section className={styles.marketScene}>
      <header className={styles.skillHero}>
        <div>
          <span>{TEACHERIN_BRAND.shortName} Skills</span>
            ...
            {addMenuOpen ? (
              <div className={styles.skillAddMenu} role="menu" aria-label="添加技能方式">
                <button type="button" role="menuitem" onClick={() => { setAddMenuOpen(false); onFind(); }}>
                  <Search size={16} />
                  <span><strong>查找技能</strong><small>让 {TEACHERIN_BRAND.shortName} 帮你找到合适的 Skill</small></span>
                </button>
...
function StandaloneSettingsSurface() {
  ...
  const labels: Readonly<Record<string, Readonly<{ title: string; description: string }>>> = Object.freeze({
    ...
    model: { title: 'AI 能力', description: `当前使用 ${TEACHERIN_BRAND.shortName} 托管的 AI 能力，不需要配置个人密钥。` },
    ...
    about: { title: '关于', description: `${TEACHERIN_BRAND.officialName} 是面向教师的任务型 AI 教学工作台。` },
  ...
  return (
    <main className={styles.settingsPage} aria-labelledby="settings-workspace-title">
      ...
      <nav className={styles.settingsNav} aria-label={`${TEACHERIN_BRAND.shortName} 个人设置分组`}>
...
function SettingsSurface() {
  ...
  return (
    ...
    >
      ...
      <nav className={styles.settingsNav} aria-label={`${TEACHERIN_BRAND.shortName} 设置分组`}>
    ...
    return (
      <div className={styles.feedbackForm}>
        <p className={styles.notice}>
          帮助我们持续改善 {TEACHERIN_BRAND.shortName}。提交前请移除学生敏感信息。
        </p>
        <Field label="反馈类型">
          <div className={styles.radioRow}>
            <label>
  ...
  if (section === "about")
    return (
      <div className={styles.aboutPanel}>
        <Sparkles size={32} />
        <h3>{TEACHERIN_BRAND.officialName}</h3>
        <p>面向教师的任务型 AI 工作台</p>
        <dl>
          <dt>当前版本</dt>
          <dd>v0.1</dd>
