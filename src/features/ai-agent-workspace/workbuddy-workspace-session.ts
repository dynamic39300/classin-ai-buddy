...
import {
  isTaskAssemblySession,
  taskAssemblyModule,
  type TaskAssemblySession,
  type TaskAssemblySnapshot,
} from '@domain/workbuddy/task-assembly';
import type { CoursewarePanel, PackagePanel } from './workbuddy-workspace';

const STORAGE_KEY = 'workbuddy:workspace-session:v4';
const LEGACY_STORAGE_KEY = 'workbuddy:workspace-session:v3';

function storageKey(base: string, namespace = 'ideal-full'): string {
  return namespace === 'ideal-full' ? base : `${base}:${namespace}`;
}
...
type LegacyWorkBuddyWorkspaceSession = Readonly<
  Omit<WorkBuddyWorkspaceSession, 'version' | 'taskAssembly' | 'taskAssemblySnapshotsById'> & {
    version: 3;
    draftGoal: string;
  }
>;
...
function isTaskAssemblySnapshot(value: unknown): value is TaskAssemblySnapshot {
  return isRecord(value) && hasStrings(value, ['id', 'createdAt', 'goal'])
    && value.version === 'task-assembly-v1' && Array.isArray(value.materials)
    && Array.isArray(value.agentPromptProvenance);
}
...
function isWorkspaceSession(value: unknown): value is WorkBuddyWorkspaceSession | LegacyWorkBuddyWorkspaceSession {
  if (!isRecord(value) || ![3, 4].includes(Number(value.version))) return false;
  const shapeValid = isContextProposal(value.contextProposal)
    ...
    && (value.version === 3
      ...
      : isTaskAssemblySession(value.taskAssembly) && isRecord(value.taskAssemblySnapshotsById)
        && Object.values(value.taskAssemblySnapshotsById).every(isTaskAssemblySnapshot));
  ...
  if (value.version === 4 && isRecord(value.taskAssemblySnapshotsById)
    && Object.entries(value.taskAssemblySnapshotsById).some(([id, snapshot]) => !isRecord(snapshot) || snapshot.id !== id)) return false;
  if (value.version === 4 && isRecord(value.taskAssemblySnapshotsById)) {
    const taskAssemblySnapshotsById = value.taskAssemblySnapshotsById;
    const linkedAssemblySnapshotIds = [coursewareRun, packageRun, isRecord(value.quizRun) ? value.quizRun : null]
      .map((run) => run?.taskAssemblySnapshotId)
      .filter((id): id is string => typeof id === 'string');
    if (linkedAssemblySnapshotIds.some((id) => !isRecord(taskAssemblySnapshotsById[id]))) return false;
  }
  return true;
}
...
export function loadWorkBuddyWorkspaceSession(namespace = 'ideal-full'): WorkBuddyWorkspaceSession | null {
  ...
  try {
    const currentRaw = window.sessionStorage.getItem(storageKey(STORAGE_KEY, namespace));
    const legacyRaw = window.sessionStorage.getItem(storageKey(LEGACY_STORAGE_KEY, namespace));
    const value: unknown = JSON.parse(currentRaw ?? legacyRaw ?? 'null');
    if (!isWorkspaceSession(value)) return null;
    if (value.version === 4) {
      return Object.freeze({
        ...value,
        taskAssembly: taskAssemblyModule.create(value.taskAssembly),
        taskAssemblySnapshotsById: Object.freeze({ ...value.taskAssemblySnapshotsById }),
      });
    }
    const { draftGoal, ...legacy } = value;
    const initialAssembly = draftGoal
      ? taskAssemblyModule.dispatch(taskAssemblyModule.create(), {
          type: 'replace-text', value: draftGoal, caret: draftGoal.length,
        }).session
      : taskAssemblyModule.create();
    return Object.freeze({
      ...legacy,
      version: 4,
      taskAssembly: initialAssembly,
      taskAssemblySnapshotsById: Object.freeze({}),
    });
  } catch {
    return null;
  }
...
export function saveWorkBuddyWorkspaceSession(session: WorkBuddyWorkspaceSession, namespace = 'ideal-full'): void {
  ...
  window.sessionStorage.setItem(storageKey(STORAGE_KEY, namespace), JSON.stringify(session));
}

export function clearWorkBuddyWorkspaceSession(namespace = 'ideal-full'): void {
  if (typeof window === 'undefined') return;
  window.sessionStorage.removeItem(storageKey(STORAGE_KEY, namespace));
  window.sessionStorage.removeItem(storageKey(LEGACY_STORAGE_KEY, namespace));
}
