...
export const WORKBUDDY_CAPABILITIES: readonly WorkBuddyCapability[] = [
  ...
  { id: 'files', label: '我的文件', description: '回溯 Work Buddy 在 ClassIn Space 中保存的 AI 协作产物。', icon: FolderOpen, placement: 'resource' },
...
export function getWorkBuddyCapabilityFromPathname(pathname: string) {
  const section = pathname.match(/^\/teacher\/ai-agent\/([^/]+)/)?.[1];
  return section ? getWorkBuddyCapability(section) : undefined;
}
