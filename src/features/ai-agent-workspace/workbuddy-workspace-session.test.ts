...
import { createSingleCoursewareRun } from '@domain/workbuddy/course-production';
import { taskAssemblyModule } from '@domain/workbuddy/task-assembly';
import { WORKBUDDY_COURSE_PACKAGE_DEFINITION, WORKBUDDY_COURSEWARE_DEFINITION } from '@mocks/scenarios/workbuddy-course-production';
...
const CURRENT_STORAGE_KEY = 'workbuddy:workspace-session:v4';
...
describe('WorkBuddy workspace session boundary', () => {
  ...
  it('accepts a structurally valid empty workspace session', () => {
    ...
    expect(loadWorkBuddyWorkspaceSession()).toMatchObject({ version: 4, taskType: 'single-courseware', taskAssembly: { text: '' } });
  });
  ...
  it('never falls back across ideal and MVP workspace namespaces', () => {
    ...
    expect(loadWorkBuddyWorkspaceSession('ideal-full')?.taskAssembly.text).toBe('终局任务');
    expect(loadWorkBuddyWorkspaceSession('classin-mvp')?.taskAssembly.text).toBe('MVP 任务');
  ...
  it('fails closed when a Run links to a missing task assembly Snapshot', () => {
    const legacy = validSession();
    const contextSnapshot = {
      id: 'context-courseware-assembly',
      version: 'workbuddy-m4-context-v1',
      taskType: 'single-courseware',
      confirmedAt: '2026-09-03T10:00:00+08:00',
      items: legacy.contextProposal.items,
    };
    const session = {
      ...legacy,
      version: 4,
      contextSnapshot,
      snapshotsById: { [contextSnapshot.id]: contextSnapshot },
      coursewareRun: createSingleCoursewareRun(
        WORKBUDDY_COURSEWARE_DEFINITION,
        '生成课件',
        contextSnapshot.id,
        'assembly-missing',
      ),
      taskAssembly: taskAssemblyModule.create(),
      taskAssemblySnapshotsById: {},
    };

    window.sessionStorage.setItem(CURRENT_STORAGE_KEY, JSON.stringify(session));
    expect(loadWorkBuddyWorkspaceSession()).toBeNull();
  });
  ...
  it('persists a valid TeacherIn draft receipt outside the main Run session', () => {
    const receipt = {
      ...
      draft: {
        ...
        sourceSpaceFileRef: { id: 'space-file-1', version: 'v1', pathLabel: '我的云盘 / TeacherIn 产物' },
      },
    };
