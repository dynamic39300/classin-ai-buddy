...
function projectQuizHistory(current: readonly WorkBuddyRunViewModel[], run: QuizActivityCreationRun, fixture: WorkBuddyRuntimeFixture): readonly WorkBuddyRunViewModel[] {
  ...
  const item: WorkBuddyRunViewModel = {
    ...
    goal: run.goal, contextLabels: existing?.contextLabels ?? [],
    steps: [{ title: run.stage === 'artifact_saved' ? '测验试卷已保存' : completed ? '测验活动草稿已创建' : run.artifact ? '测验试卷已生成' : '等待确认试卷结构', summary: run.stage === 'artifact_saved' ? '已保存到当前账号的个人内容库。' : completed ? '请前往班级课程详情审阅并发布。' : 'TeacherIn 只会创建草稿，不会发布。', time: fixture.history.currentStepTime, state: completed ? 'completed' : failed ? 'failed' : 'waiting' }],
