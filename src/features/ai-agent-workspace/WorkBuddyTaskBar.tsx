...
export function WorkBuddyTaskBar({ contextPanel, showReturnCommand = true }: Readonly<{
  ...
  return (
    <header ref={taskBarRef} className={styles.taskBar} aria-label={`${TEACHERIN_BRAND.shortName} 任务导航`} data-has-return={showReturnCommand && profile.returnTarget ? 'true' : undefined}>
      ...
      <nav ref={tabViewportRef} className={styles.tabViewport} aria-label={`已打开的 ${TEACHERIN_BRAND.shortName} 任务`}>
        {openTabs.map((tab) => {
