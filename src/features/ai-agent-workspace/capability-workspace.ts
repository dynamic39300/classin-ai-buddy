...
import type { TaskAgentReference, TaskSkillReference } from "@domain/workbuddy/task-assembly";
...
export type TaskSkillOption = TaskSkillReference;
...
export const TASK_SKILL_OPTIONS: readonly TaskSkillOption[] = [
  {
    kind: "skill", id: "skill-single-courseware", title: "生成单个课件",
    description: "生成包含概念讲解、例题和课堂练习的智能课件", source: "official",
    taskIntent: { taskType: "single-courseware", suggestedGoal: "为高一（3）班生成一份函数单调性智能课件，包含概念讲解、例题和课堂练习" },
  },
  {
    kind: "skill", id: "skill-course-package", title: "生成课程方案包",
    description: "生成课件、作业、测验和录播脚本的完整方案", source: "official",
    taskIntent: { taskType: "course-package", suggestedGoal: "从函数单调性课程目标出发，生成包含课件、作业、测验和录播脚本的课程方案包" },
  },
  {
    kind: "skill", id: "skill-quiz-activity", title: "生成测验",
    description: "生成诊断测验并创建为教学活动草稿", source: "official",
    taskIntent: { taskType: "quiz-activity-creation", suggestedGoal: "为高二物理 3 班当前单元生成一份动量守恒诊断测验，并创建为教学活动草稿" },
  },
  {
    kind: "skill", id: "skill-class-insight", title: "分析班级学情",
    description: "归纳作业共性问题并生成教学建议", source: "official",
    taskIntent: { taskType: "single-courseware", suggestedGoal: "分析高一（3）班最近一次作业，归纳共性问题并给出教学建议" },
  },
  {
    kind: "skill", id: "skill-word-document", title: "Word 文档",
    description: "生成结构清晰、可交付的 Word 文档草稿", source: "official", taskIntent: null,
  },
];

export const TASK_AGENT_OPTIONS: readonly TaskAgentReference[] = [
  {
    kind: "agent", id: "daily-quote", title: "每日名言", source: "agentin",
    avatarAsset: "/reference/agentin/avatars/daily-quote.png",
    recommendedPrompt: "请结合当前课程主题，推荐一句适合学生的每日名言，并解释其含义与课堂使用方式。",
  },
  {
    kind: "agent", id: "idiom-origin", title: "成语溯源与应用专家", source: "agentin",
    avatarAsset: "/reference/agentin/avatars/idiom-origin.png",
    recommendedPrompt: "请结合当前课程内容，选择一个相关成语，说明出处、含义，并设计一个适合学生的应用练习。",
  },
  {
    kind: "agent", id: "geography-encyclopedia", title: "地理百科大全", source: "agentin",
    avatarAsset: "/reference/agentin/avatars/geography-encyclopedia.png",
    recommendedPrompt: "请围绕当前教学主题补充准确的地理背景、关键概念和适合学生理解的案例。",
  },
  {
    kind: "agent", id: "confucius", title: "孔子", source: "agentin",
    avatarAsset: "/reference/agentin/avatars/confucius.png",
    recommendedPrompt: "请从孔子及儒家思想的角度解读当前主题，并给出适合课堂讨论的问题。",
  },
];

export const LEGACY_TASK_SKILL_OPTIONS: readonly TaskSkillOption[] = [
  { kind: "skill", id: "skill-word-document", title: "Word 文档", description: "生成结构清晰、可交付的 Word 文档草稿", source: "official", taskIntent: null },
  { kind: "skill", id: "skill-web-design", title: "网页设计", description: "设计并生成可预览的网页内容", source: "official", taskIntent: null },
  { kind: "skill", id: "skill-file-upload", title: "文件上传", description: "读取本次任务中明确选择的文件资料", source: "official", taskIntent: null },
  { kind: "skill", id: "skill-pdf-document", title: "PDF 文档", description: "生成适合阅读与分发的 PDF 文档", source: "official", taskIntent: null },
  { kind: "skill", id: "skill-presentation", title: "PPT 演示文稿", description: "生成多页课件与演示文稿草稿", source: "official", taskIntent: null },
  { kind: "skill", id: "skill-subject-expert", title: "学科专家", description: "按学科知识与教学目标辅助任务规划", source: "official", taskIntent: null },
];
...
const CAPABILITY_SURFACE_IDS: readonly CapabilitySurfaceId[] = [
  "skills", "tools", "content", "files", "schedules", "settings",
];

export const CAPABILITY_SURFACE_CONFIGS: readonly CapabilitySurfaceConfig[] =
  CAPABILITY_SURFACE_IDS.map((id) => {
    const capability = WORKBUDDY_CAPABILITIES.find((item) => item.id === id);
    if (!capability) throw new Error(`Missing WorkBuddy capability registry entry: ${id}`);
    return {
      id,
...
export const SKILL_ITEMS: readonly CapabilityItem[] = [
  {
    ...
    source: TEACHERIN_BRAND.officialName,
    version: "v1.4.0",
...
export const FILE_ITEMS: readonly CapabilityItem[] = [
  {
    ...
    source: `${TEACHERIN_BRAND.shortName} Artifact`,
    version: "v2",
...
export function filterCapabilityItems(
  ...
  return items.filter((item) => {
    ...
    if (tab === "artifacts") return item.source === `${TEACHERIN_BRAND.shortName} Artifact`;
