    ...
    <section className={styles.page} data-panel-open={activePanel !== 'none'} aria-labelledby="m4-courseware-title">
          ...
          {run.showPlan ? (
            <section className={styles.stageCard} aria-labelledby="courseware-plan-title">
              ...
              <div className={styles.stageBody}>
                ...
                <ol className={styles.planList}>{run.plan.map((step) => <li key={step.id}><div><strong>{step.title}</strong><span>{step.expectedOutput}</span><small>能力摘要：{step.capabilitySummary}</small></div></li>)}</ol>
      ...
      {activePanel === 'replan' ? (
        <aside className={styles.panel} aria-label="重新规划影响">
          ...
          <footer><button type="button" onClick={closePanel}>保留当前范围</button><button className={styles.primaryPanelAction} type="button" onClick={() => { replanToWaveContext(); requestAnimationFrame(() => lastPanelTriggerRef.current?.focus()); }}>确认并重新规划</button></footer>
        </aside>
      ) : null}
        ...
        <aside className={styles.panel} aria-label="执行回执">
          ...
          <div className={styles.receiptBody}>
            {coursewareReceipt.status === 'success' ? <>
              ...
              </dl><p>{coursewareReceipt.truthLabel}</p><details><summary>技术证据</summary><code>{coursewareReceipt.id} · {coursewareReceipt.actionId} · {coursewareReceipt.approvalId}</code></details>
              ...
              <dl>
                <div><dt>未执行范围</dt><dd>{coursewareAction?.target.label ?? '原保存目标'}</dd></div>
                {coursewareReceipt.status === 'version_conflict' ? <div><dt>版本比较</dt><dd>提交版本：{coursewareReceipt.expectedVersion} · 当前版本：{coursewareReceipt.currentVersion}</dd></div> : null}
                <div><dt>恢复方式</dt><dd>{coursewareReceipt.status === 'permission_denied' ? '选择其他可写入位置' : coursewareReceipt.status === 'version_conflict' ? '比较版本并重新确认，禁止静默覆盖' : '已保留审批记录，可安全重试'}</dd></div>
              </dl>
              <p>{coursewareReceipt.truthLabel}</p><details><summary>技术证据</summary><code>{coursewareReceipt.unexecutedTarget} · {coursewareReceipt.approvalId} · {coursewareReceipt.idempotencyKey}</code></details>
