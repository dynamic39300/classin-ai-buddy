...
describe("WorkBuddy capability workspace model", () => {
  ...
  it('exposes only active destinations while retaining the dormant content module', () => {
    expect(WORKBUDDY_VISIBLE_CAPABILITIES.map(({ id }) => id)).toEqual([
      'agentin', 'skills', 'tools', 'files', 'schedules', 'settings',
    ]);
  ...
  it("exposes the installed official skills shown in the task picker", () => {
    ...
    expect(TASK_SKILL_OPTIONS.every(({ source }) => source === "official")).toBe(true);
  });

  it("exposes the four AgentIn references shown in the task picker", () => {
    expect(TASK_AGENT_OPTIONS.map(({ title }) => title)).toEqual([
      "每日名言",
      "成语溯源与应用专家",
      "地理百科大全",
      "孔子",
    ]);
    expect(TASK_AGENT_OPTIONS.every(({ source }) => source === "agentin")).toBe(true);
    expect(TASK_AGENT_OPTIONS.every(({ recommendedPrompt }) => recommendedPrompt.length > 0)).toBe(true);
  });
