...
export function createWorkBuddyPackageController(params: PackageControllerParams): WorkBuddyPackageController {
  ...
  return Object.freeze({
    ...
    commands: Object.freeze({
      ...
      createPackageTask: (goal: string, taskAssemblySnapshotId: string | null = null) => {
        if (!contextSnapshot || taskType !== 'course-package' || !goal.trim()) return null;
        const next = createCoursePackageRun(packageDefinition, goal, contextSnapshot.id, undefined, taskAssemblySnapshotId);
      ...
      derivePackageFromCourseware: () => {
        ...
        }, sourceCoursewareRun.taskAssemblySnapshotId);
