...
export function MessageWorkspaceResizableLayout({
  ...
  const applyWidth = useCallback((nextWidth: number) => {
    ...
    if (separator) {
      ...
      separator.setAttribute('aria-valuetext', `${TEACHERIN_BRAND.shortName} 宽度 ${Math.round(nextWidth)} 像素`);
    }
  }, []);
      ...
      {assistant ? (
        <>
          <div
            ...
            aria-label={`调整 ${TEACHERIN_BRAND.shortName} 宽度`}
            aria-orientation="vertical"
            aria-valuemax={Math.round(limits.max)}
            aria-valuemin={Math.round(limits.min)}
            aria-valuenow={Math.round(committedWidth)}
            aria-valuetext={`${TEACHERIN_BRAND.shortName} 宽度 ${Math.round(committedWidth)} 像素`}
