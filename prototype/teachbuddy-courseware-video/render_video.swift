...
let scenes: [Scene] = [
    Scene(content: .title, duration: 1.8, kicker: "CLASSIN TEACHBUDDY", title: "从教学目标到可用课件", detail: "30 秒看完一次可审阅、可批准、可追溯的教师 AI 任务", highlight: nil, callout: nil),
    ...
    Scene(content: .end, duration: 2.2, kicker: "TEACHBUDDY BUSINESS LOOP", title: "从“会生成”走到“能完成”", detail: "目标 → Context → Artifact → 审阅 → Approval → Receipt", highlight: nil, callout: nil),
]
...
func drawTitleCard(kicker: String, title: String, detail: String, end: Bool) {
    ...
    if end {
        ...
        drawText("真实 Demo 路径 · 站内终局 TeachBuddy", in: CGRect(x: 78, y: 472, width: 760, height: 30), font: .boldSystemFont(ofSize: 19), color: NSColor(calibratedRed: 0.20, green: 0.90, blue: 0.65, alpha: 1))
    }
}
...
for frameIndex in 0..<totalFrames {
    ...
    if !adaptor.append(pixelBuffer, withPresentationTime: presentationTime) {
        throw writer.error ?? NSError(domain: "TeachBuddyVideo", code: 1)
    }
}
...
if writer.status != .completed {
    throw writer.error ?? NSError(domain: "TeachBuddyVideo", code: 2)
}
